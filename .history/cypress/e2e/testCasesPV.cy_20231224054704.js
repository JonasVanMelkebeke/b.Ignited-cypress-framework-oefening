...
describe('Test Cases Proces Verbaal nummers', () => {
  ...
  it('[Proces Verbaal nummers] Verify functionality of button generate - Default inputs return only 1 number', () => {
    ...
    testDataGeneratorObj.pvGeneratedNumbersTxt.invoke('text').then((text) => {
      const lines = pvGeneratedNumbersTxt.split('\n');
      // Check the number of lines
      expect(lines).to.have.length(expectedReturnedNumbers);
    });
  })    
 })