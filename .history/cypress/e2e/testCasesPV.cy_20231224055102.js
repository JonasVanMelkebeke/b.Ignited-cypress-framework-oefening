...
describe('Test Cases Proces Verbaal nummers', () => {
  ...
  it('[Proces Verbaal nummers] Verify functionality of button generate - Default inputs return only 1 number', () => {
    ...
    cy.waitUntil(() => {
      return testDataGeneratorObj.pvGeneratedNumbersTxt.invoke('text').then((text) => {
        const lines = text.split('\n');
        return lines.length === expectedReturnedNumbers;
      });
    }, { timeout: 10000, interval: 1000 }); // Adjust timeout and interval as needed
  })    
 })