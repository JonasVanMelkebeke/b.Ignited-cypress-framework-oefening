...
describe('Test Cases Proces Verbaal nummers', () => {
  ...
  it('[Proces Verbaal nummers] Verify functionality of button generate - Default inputs return only 1 number', () => {
    ...
    cy.get(testDataGeneratorObj.elements.pvGeneratedNumbersTxt)
      .invoke('text')
      .then((text) => {
        console.log('pvGeneratedNumbersTxt Content:', text); // Log the entire content
  
        const numberOfMatches = (text.match(new RegExp(`${expectedSuffix}`, 'g')) || []).length;
        console.log('Number of Matches:', numberOfMatches); // Log the number of matches
  
        expect(numberOfMatches).to.equal(expectedReturnedNumbers);
      });
  });
 })